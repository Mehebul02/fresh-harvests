...
export const authApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getCurrentUser: builder.query({
      query: () => {
        const token = (state: RootState) => state.auth.token; // Get token from Redux state
        console.log("Token from Redux:", token); // Check if the token is correctly being retrieved
        return {
          url: '/auth/profile',
          method: 'GET',
          headers: {
