...
type USerProps = {
  ...
  expires?: string; // Add the 'expires' property to match the Session type
};

const Navbar = ({session}: {session: USerProps  & { expires: string  } | null}) => {
  ...
  const { user, setIsLoading } = useUse();
  return (
...
{
  session?.user ? (
    <ProfilePopup session={{ ...session, expires: session.expires || "" }} />
  ) : (
    <LoginForm login="Sign In" />
  )
}
