...
console.log(data);
  ...
  const handleLogout = () => {
    dispatch(logout()); // Redux action to clear auth state
  localStorage.removeItem('token'); // টোকেন সরান
  localStorage.removeItem('persist:root'); // Redux Persist ডাটা ক্লিয়ার করুন
  router.push('/login');
  };
