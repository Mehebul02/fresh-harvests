      ...
      <div className="flex flex-wrap justify-center sm:flex-nowrap space-x-4 bg-gray-100 p-2 rounded-lg shadow-md max-w-[600px] mx-auto">
        <button
          ...
          className={`w-full sm:w-auto px-6 py-2 rounded-lg text-gray-600 font-medium transition-all duration-300 ${
            activeTab === "All" ? "bg-[#749B3F] cursor-pointer text-white shadow-md" : "hover:bg-gray-200"
          }`}
        >
          All
        </button>
        {categories?.data?.map((category: ICategory) => (
          <button
            ...
            className={`w-full sm:w-auto px-6 py-2 cursor-pointer rounded-lg text-gray-600 font-medium transition-all duration-300  ${
              activeTab === category.id
                ? "bg-[#749B3F] text-white shadow-md"
                : "hover:bg-gray-200"
            }`}
          >
     ...
     <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 max-w-[2940px] mx-auto">
        {filteredProducts.length > 0 ? (
          filteredProducts.slice(0, 8)?.map((product) => (
            ...
            <div
              className="bg-white rounded-lg shadow-lg p-4 text-center border border-gray-200 transform transition-transform duration-300 hover:scale-105 hover:shadow-xl flex flex-col justify-between min-h-[300px]"
            >
              {/* Product Image */}
              ...
              {/* Product Name */}
              <h3 className="text-lg font-semibold text-gray-800 mt-2 line-clamp-2 h-[50px]">
                {product.productName}
              </h3>
