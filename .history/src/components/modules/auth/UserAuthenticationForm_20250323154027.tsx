...
const LoginForm = ({ login }: ILogin) => {
  ...
  import jwtDecode from 'jwt-decode'; // jwt-decode প্যাকেজ ইনস্টল করুন
    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (res?.success && res?.data?.token) {
          const decodedToken = jwtDecode(res.data.token); // Token ডিকোড করুন
          const user = {
            id: decodedToken.id,
          ...
          dispatch(setUser({ user, token: res.data.token })); // Dispatch setUser action
