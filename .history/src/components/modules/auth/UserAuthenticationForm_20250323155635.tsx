  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (isSignUp) {
        ...
        console.log('Register Response:', res); // Check structure here
        if (res?.success) {
        ...
        console.log('Login Response:', res); // Check structure here
        if (res?.success && res?.data?.token) {
          // Save the token to localStorage
          localStorage.setItem("token", res?.data?.token);
  
          // Ensure fullName and email are present
          const userData = res?.data;
          // Check if fullName and email are available in a different structure
          const fullName = userData?.user?.fullName || userData?.full_name;  // Modify based on your response structure
          const email = userData?.user?.email || userData?.email;
  
          if (fullName && email) {
            // Dispatch the user information including fullName and email
            dispatch(setUser({
              user: fullName,
              email: email,  
