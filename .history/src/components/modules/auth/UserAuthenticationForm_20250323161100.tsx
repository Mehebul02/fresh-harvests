    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (res?.success && res?.data?.token) {
          ...
          // Ensure that the response contains fullName, email, and token
          const { fullName, email, token } = res?.data;

          if (fullName && email && token) {
            // Check if fullName (userName) is null and handle it
            const userName = fullName || "Guest"; // Fallback to "Guest" if userName is null
          
            // Dispatch the user information with all required properties
            dispatch(setUser({
              userName: userName, // Fallback to "Guest" if fullName is null
              email: email, // Add email here
              token: token, // Make sure token is present
            }));
