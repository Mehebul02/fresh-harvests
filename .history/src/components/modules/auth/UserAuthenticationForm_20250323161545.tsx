    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (res?.success && res?.data?.token) {
          ...
          // Check for missing userName and use fallback value if null
          const { userName, email, token } = res?.data;
          const userFullName = userName || email || "Guest"; // Fallback to email or "Guest"
  
          if (email && token) {
            // Dispatch the user information with the correct userName
            dispatch(setUser({
              userName: userFullName,  // Set the userName (either userName or email)
