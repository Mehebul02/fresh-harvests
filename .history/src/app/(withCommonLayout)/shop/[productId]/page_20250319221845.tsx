...
const ProductDetailsPage = () => {
  ...
  if (isLoading || isLoadingCategories) return <div className="text-center py-10">Loading...</div>;
  ...
  const relatedProducts = categories?.data?.filter(
    (item: any) => item.category === product?.data?.category && item.id !== product?.data?.id
  );
