...
const AddProductPage = () => {
  ...
  const { data: categoryOptions } = useGetCategoryQuery(undefined);

  const onSubmit = async (data: ProductFormData) => {
    try {
      const response = await addProduct(data).unwrap(); // API Call
  ...
  return (
    <div>
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-5xl mx-auto bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="mt-2">
            {imageUrls.map((url, index) => (
              <div key={index} className="flex items-center space-x-2">
                <Image src={url} alt="Product" className="w-16 h-16 object-cover rounded" width={64} height={64} />
        ...
        {/* Submit Button */}
        <Button  type="submit" className="w-full mt-4 cursor-pointer bg-[#749B3F] hover:bg-[#959b3f]" disabled={isLoading}>
